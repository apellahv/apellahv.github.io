		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				setTimeout(function() {
					...
					$console_message.text('Loading scripts...');
					progressBarConsole(3, $('#progressBarConsole'));
				}, 0 );
				setTimeout(function() {
					$console_message.text('Launching scripts...');
				...
				setTimeout(function() {
					$console_message.html("Searching for submitted username <span class='console-message-connected-item'>#" + $console_message_username_msg + "</span> ...");
					progressBarConsole(22, $('#progressBarConsole'));
				}, 3000 );
				setTimeout(function() {
					$console_message.html("Connecting to servers <span class='console-message-connected-item'>#" + $console_message_username_msg + "</span> on <span class='console-message-connected-item'>" + $console_message_platform_msg + "</span> Platform");	
				...
				setTimeout(function() {
					$console_message.html("Successfully connected to server <span class='console-message-connected-item'>#" + $console_message_username_msg + "</span>");
				...
				setTimeout(function() {
					$console_message.html("Preparing Gold...");
					progressBarConsole(35, $('#progressBarConsole'));
				}, 10000 );
				setTimeout(function() {
					$console_message.html("Generating Gold");
					progressBarConsole(38, $('#progressBarConsole'));
				}, 11000 );
				setTimeout(function() {
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_gold_msg + "</span> Generating Gold");
				...
				setTimeout(function() {
					$console_message.html("Preparing Gems...");
					progressBarConsole(58, $('#progressBarConsole'));
				}, 18000 );
				setTimeout(function() {
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_gems_msg + "</span> Generating Gems");
				...
				setTimeout(function() {
					$console_message.html("<span class='console-message-success'>All Resources Successfully Generated!</span>");
					progressBarConsole(80, $('#progressBarConsole'));
				}, 24900 );
				setTimeout(function() {
					$console_message.html("Optimizing actions...");
					progressBarConsole(84, $('#progressBarConsole'));
				}, 25900 );
				setTimeout(function() {
					$console_message.html("Sending to server...");
					progressBarConsole(90, $('#progressBarConsole'));
				}, 27000 );
				setTimeout(function() {
					$console_message.html("Confirm your device for verification...");
				...
				setTimeout(function() {
					$console_message.html("<span class='console-message-connected-item'>Manual verification required!</span>");
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
					progressBarConsole(93, $('#progressBarConsole'));
				...
				setTimeout(function() {
					...
					$console_message.html("Awaiting Human Verification!");
					if ($(window).width() < 600) {
...
var ChatContent = ["nice this really works", "works perfectly", "great website", "WOW thank you for the 50,000 gems", "really amazing website", "that's dope, was looking for such a hack!", "this website is indeed gr8", "I've just received 30k gems lol ^^", "hey ppl", "hello!", "good shit, something that finally works", "one word, perfect!", "I'm so thankful this website exists", "just watched a vid on this hack on youtube", "i tested it, it works as promised", "thank you! ! !!", "yep after verifying i got both gems and gold", "was looking for something that works for weeks, so glad i found this ", "this does work i can vouch for it", "hooorayy! it works so good, alrdy used it 2-3 times", "so nice that it works", "thx!", "Yoooo guys", "Hello people on chat", "what a cool website!", "after verification igot my resources", "i also got my resources after verifying", "thanksss this works amazingly", "hi", "works like a god church lol", "glad i didnt waste my time with this and actually got smt out of it", "Omfg i wonder if i can use it a lot of times within a few minutes? anybody knows maybe?", "Dayum it works", "Brooski ty for this croyale hack!", "words cant even describe how happy i am it actually worked", "coolio, it worked xD", "yay bitch! I got my gems!", "Dude or dudes.. You are amazing for making this cheat tool!", "Shit i need to relax, i m so happy it worked", "u serious!? free and no risks this is what  i was lf!", "hello", "hey", "it actually works! holy shit!", "didnt have much hopes going in and trying this but it does work after all lol"];
var ChatAntiBot = ["no we are not bots hah lol", "no dude we're not bots >.<", "we aint bots dummy, chat works amazingly.. Just try it", "of course we are real dummy", "if u actually think we are bots u are dumb af dude"];
...
$(document).ready(function() {
    ...
    ChatStart();
    ChatLog("Welcome to the chat, please avoid posting links to other websites.");
    ...
    $('#livechatButtonChat')['click'](function() {
        if (ChatUserName == '') {
            ...
            if (!ChatReplied) {
                setTimeout(function() {
                    ChatAddEntry(ChatUserNames[Random(0, ChatUserNames['length'] - 1)], '<span class="mention">@ ' + ChatUserName + ' &nbsp;</span>  lol stop with the spam and just use the website for what it is meant to be used.');

                    setTimeout(function() {
                        ChatAddEntry(ChatUserNames[Random(0, ChatUserNames['length'] - 1)], '<span class="mention">@ ' + ChatUserName + ' &nbsp;</span>  Is this your first time here? Please note that this is the only real working hack for Clash Royale out there.');
                        setTimeout(function() {
                            ChatAddEntry(ChatUserNames[Random(0, ChatUserNames['length'] - 1)], 'dont listen to ' + '<span class="mention">@ ' + ChatUserName + ' &nbsp;</span> ' + ' He wants all keep all the gold and gems to himself');
